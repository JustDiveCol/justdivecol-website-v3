...
interface ActiveDestinationCardProps {
  ...
  className?: string; // <-- Añadimos la prop
}
...
export const ActiveDestinationCard = ({
  ...
  return (
    ...
    <Link
      ...
      className={twMerge(
        'group relative block w-full overflow-hidden rounded-lg shadow-lg aspect-[4/5]',
        className
      )}>
      <div className='absolute inset-0 transition-transform duration-500 ease-in-out group-hover:scale-110'>
        <ImageComponent
          imageData={{ backgroundImage: destination.header.backgroundImage }}
        />
      </div>
