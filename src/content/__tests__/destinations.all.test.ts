...
import type { DestinationContent } from '../destinations/types';
...
function isDestinationContent(obj: any): obj is DestinationContent {
  ...
  const uf = obj.uniqueFinds;
  if (
    !uf ||
    !isNonEmptyString(uf.titleKey) ||
    !Array.isArray(uf.items) ||
    uf.items.length < 1 ||
    !uf.items.every(isNonEmptyString)
  )
    return false;
...
describe('Destinations: all *.content.ts files', async () => {
  const files = await fg(['destinations/**/*.content.ts'], {
    cwd: CONTENT_ROOT,
    absolute: true,
  });

  it('encuentra al menos un archivo de destinations', () => {
  ...
  for (const abs of files) {
    ...
    it(`valida DestinationContent en ${rel}`, async () => {
      const mod = await import(abs);
      const candidates = Object.values(mod);

      // Busca el primer export que parezca DestinationContent
      const found = candidates.find(isDestinationContent);
      expect(
        found,
        `No se encontró un export con shape DestinationContent en ${rel}`
      ).toBeTruthy();

      const dest = found as DestinationContent;

      // Chequeos extra útiles:
      // - details.items con label/value no vacíos
      expect(
        dest.details.items.every(
          (it) => it.labelKey.trim() && it.valueKey.trim()
        )
      ).toBe(true);
      // - description.paragraphs no vacíos
      expect(
        dest.description.paragraphs.every((p) => p.trim().length > 0)
      ).toBe(true);
      // - uniqueFinds.items no vacíos
      expect(dest.uniqueFinds.items.every((i) => i.trim().length > 0)).toBe(
        true
      );
    });
  }
