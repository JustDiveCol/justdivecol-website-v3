...
import { policyContent } from '../pages/legal/policy/policy.content';
...
describe('Legal Policy content', () => {
  ...
  it('tiene las secciones esperadas', () => {
    expect(policyContent).toHaveProperty('seo');
    expect(policyContent).toHaveProperty('header');
    expect(policyContent).toHaveProperty('content');
  });

  it('seo básico válido', () => {
    const { seo } = policyContent as any;
  ...
  it('header: estructura e imagen', () => {
    const { header } = policyContent as any;
  ...
  it('content: sections, points y subpoints válidos', () => {
    const content = (policyContent as any)?.content;
